...
create(Server, File) ->
    Ref = make_ref(),
    gen_server:cast(Server, {create, File, self(), Ref}),
    Ref.
...
%% @end
%%--------------------------------------------------------------------
handle_cast({create, File, PID, Ref}, State) ->
    create_hist(State#state.port, File),
    PID ! {created_hist, Ref},
    {noreply, State};
